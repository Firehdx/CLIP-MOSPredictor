...
if __name__ == '__main__':
    ...
    image_feats = extract_all_images(
        image_paths, model, device, batch_size=64)
    ...
    if args.references_json:
        ...
        #print('{CLIPScore}: {}'.format([s['CLIPScore'] for s in scores.values()]))
    ...
    if args.references_json:
        if args.compute_other_ref_metrics:
            ...
            v = other_metrics['bleu']
            for bidx in range(len(v)):
                other_metrics['bleu-{}'.format(bidx+1)] = v[bidx]
            other_metrics.pop('bleu')
        for k, v in other_metrics.items():
            print(k+':{}'.format(v))
        print('CLIPScore: {}'.format([s['CLIPScore'] for s in scores.values()]))
        print('RefCLIPScore: {}'.format([s['RefCLIPScore'] for s in scores.values()]))

    if args.save_per_instance:
        with open(args.save_per_instance, 'w') as f:
            f.write(json.dumps(scores))

    quality_json = "AGIQA-3K/quality.json"
    align_json = "AGIQA-3K/align.json"
    with open(quality_json) as q:
        qua = json.load(q)
    with open(align_json) as a:
        ali = json.load(a)
    x = []
    y = []
    z = []
    for img in qua.keys():
        x.append(float(qua[img][0]))
        y.append(float(ali[img][0]))
        z.append(scores[img]['CLIPScore'])
